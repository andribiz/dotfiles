...
screenPositions.right = { x = HALF_GRID_SIZE - 0.25, y = 0, w = HALF_GRID_SIZE + 0.25, h = GRID_SIZE }
screenPositions.rightCode = { x = HALF_GRID_SIZE, y = 0, w = HALF_GRID_SIZE + 80, h = GRID_SIZE }
...
function This.windowMinimize(factor, window)
	if window == nil then
		window = hs.window.focusedWindow()
	end
	if window then
